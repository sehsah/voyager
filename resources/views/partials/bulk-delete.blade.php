            ...
            <div class="modal-footer">
                <form action="{{ route('voyager.'.$dataType->slug.'.index') }}/0" id="bulk_delete_form" method="POST">
                    ...
                    <input type="submit" class="btn btn-danger pull-right delete-confirm"
                             value="{{ __('voyager::generic.bulk_delete_confirm') }} {{ strtolower($dataType->title) }}">
                </form>
...
window.onload = function () {
    ...
    $bulkDeleteBtn.click(function () {
        ...
        if (count) {
            ...
            $bulkDeleteInput.val('');
            // Deletion info
            var displayName = count > 1 ? '{{ $dataType->title }}' : '{{ $dataType->getTranslatedAttribute('display_name_singular') }}';
